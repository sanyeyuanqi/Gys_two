...
const englishTranslations: Record<string, string> = {
  ...
  '页面仅显示格式示例，不会提交该内容。真实标签由后端按“用户ID-category-HHmmss”生成。': 'This page only shows a format preview and does not submit it. The backend generates the real tag as “userID-category-HHmmss”.',
  '批量上传': 'Batch Upload',
  '单个上传': 'Single Upload',
  '渠道分类': 'Channel Category',
  '（可选）': '(optional)',
  '每行一个密钥': 'One key per line',
  '标签 / 分组（仅示例）': 'Tag / Group (preview only)',
...
function buildUploadTagPreview(category: string, timestamp: number, language: Language) {
  const date = new Date(timestamp);
  const time = [date.getHours(), date.getMinutes(), date.getSeconds()]
    .map((value) => String(value).padStart(2, '0'))
    .join('');
  return `${language === 'en' ? 'preview' : '示例'}-${category}-${time}`;
}
...
function UploadView() {
  ...
  const tagPreview = useMemo(
    () => buildUploadTagPreview(category, tagTimestamp, language),
    [category, language, tagTimestamp],
  );
    ...
    <section className="upload-page">
      ...
      <div className="warning-card">
        ...
        <div>
          ...
          <p>
            {t('页面仅显示格式示例，不会提交该内容。真实标签由后端按“用户ID-category-HHmmss”生成。')}
          </p>
        </div>
      </div>
        ...
        <form className={`upload-workspace-form upload-workspace-form-${mode}`} onSubmit={submit}>
          <div className="upload-left-column">
            ...
            <label className="upload-tag-field">
              <span>
                {t('标签 / 分组（仅示例）')} <span className="field-help">?</span>
              </span>
              <span className="tag-input-wrap">
                <Tag aria-hidden="true" size={15} />
                <input
                  aria-label={t('标签 / 分组（仅示例）')}
                  disabled
                  value={tagPreview}
                />
              </span>
